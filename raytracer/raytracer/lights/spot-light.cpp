...
std::shared_ptr<lights::LightSource> raytracer::lights::spot(const math::Point3D& position, const math::Vector3D& direction, math::Angle angle, const imaging::color& color)
{
    return std::make_shared<raytracer::lights::_private_::SpotLight>(position, direction, angle, color);
}

std::shared_ptr<lights::LightSource> raytracer::lights::spot(const math::Point3D& position, const math::Point3D& pointed_at, math::Angle angle, const imaging::color& color)
{
    return std::make_shared<raytracer::lights::_private_::SpotLight>(position, (pointed_at - position).normalized(), angle, color);
}