...
#define TEST_EYE_LOOKAT(P, EYE, LOOK_AT, EXPECTED_ORIGIN, EXPECTED_DIRECTION)   TEST_CASE("[OrthographicCamera] Eye = " #EYE ", Lookat = " #LOOK_AT ", At " #P, "[OrthographicCamera]") \
                                                                                { \
                                                                                    auto camera = raytracer::cameras::orthographic(Point3D(EYE), Point3D(LOOK_AT), Vector3D(0, 1, 0), 1, 1); \
...
TEST(XY(1, 0.5), XYZ(0.5, 0, 0), XYZ(0, 0, 1))
TEST(XY(0, 0.5), XYZ(-0.5, 0, 0), XYZ(0, 0, 1))
TEST(XY(0.5, 1), XYZ(0, 0.5, 0), XYZ(0, 0, 1))
TEST(XY(0.5, 0), XYZ(0, -0.5, 0), XYZ(0, 0, 1))
