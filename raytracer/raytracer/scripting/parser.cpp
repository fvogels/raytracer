...
public:
	...
	bool has_next() const;
	std::shared_ptr<const SExpression> current() const;
	void next();

private:
	std::stack<std::list<std::shared_ptr<const SExpression>>> m_stack;
	std::stack<Location> m_locations;
};

scripting::ParsingVisitor::ParsingVisitor()
{
	m_stack.push(std::list<std::shared_ptr<const SExpression>>());
}
...
{
	auto elements = std::vector<std::shared_ptr<const SExpression>>(m_stack.top().begin(), m_stack.top().end());
...
std::shared_ptr<const SExpression> scripting::Parser::current() const
{
	assert(!end_reached());
