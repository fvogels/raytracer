...
namespace
{
    class SimpleUnionImplementation : public raytracer::primitives::_private_::PrimitiveImplementation
    {
    public:
        SimpleUnionImplementation(const std::vector<Primitive>& children)
            : m_children(children)
        {
            // NOP
        }        
...
Primitive raytracer::primitives::simple_union(const std::vector<Primitive>& children)
{
    return Primitive(std::make_shared<SimpleUnionImplementation>(children));
}
